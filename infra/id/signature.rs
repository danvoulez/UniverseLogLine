...
impl SignatureService {
    ...
    pub fn with_id(mut self, id: LogLineKeyPair) -> Self {
...
mod tests {
    ...
    fn test_verify_other_id() {
        ...
        assert!(!service.verify(&keypair2.id, data, &signature).unwrap_or(false));
        
        // Verify with the first ID (should pass)
        assert!(service.verify(&keypair1.id, data, &signature).unwrap_or(false));
    }
}